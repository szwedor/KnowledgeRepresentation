...
\begin{document}
    ...
    \begin{center}
        $\mathbf{always}/\mathbf{ever}~\mathbf{executable}~((A_1, \omega_1),...,(A_n,\omega_n))$
    \end{center}
    
    Program działań $((A_1, \omega_1),...,(A_n,\omega_n))$ jest wykonywalny zawsze/kiedykolwiek.
    
    \begin{center}
        $\mathbf{always}/\mathbf{ever}~\gamma~\mathbf{if}~\pi~\mathbf{when}~((A_1, \omega_1),...,(A_n,\omega_n))$
    \end{center}
    
    Wykonanie programu działań $((A_1, \omega_1),...,(A_n,\omega_n))$ z dowolnego stanu spełniającego  warunek $\gamma$ zawsze/kiedykolwiek prowadzi do stanu spełniającego warunek $\pi$.
    
    \begin{center}
        $\mathbf{typically}~\gamma~\mathbf{if}~\pi~\mathbf{when}~((A_1, \omega_1),...,(A_n,\omega_n))$
    \end{center}
    
    Wykonanie programu działań $((A_1, \omega_1),...,(A_n,\omega_n))$ z dowolnego stanu spełniającego  warunek $\gamma$ na ogół prowadzi do stanu spełniającego warunek $\pi$.
    
    \begin{center}
        $\mathbf{always}/\mathbf{ever}~\gamma~\mathbf{if}~\pi$
    \end{center}
    
    Wykonanie dowolnego programu działań z dowolnego stanu spełniającego warunek $\gamma$ zawsze/kiedykolwiek prowadzi do stanu spełniającego warunek $\pi$.
    
    \begin{center}
        $\mathbf{typically}~\gamma~\mathbf{if}~\pi$
    \end{center}
    
    Wykonanie dowolnego programu działań z dowolnego stanu spełniającego warunek $\gamma$ na ogół prowadzi do stanu spełniającego warunek $\pi$.
    
    \begin{center}
        $\mathbf{always}/\mathbf{ever}~\omega~\mathbf{in}~((A_1, \omega_1),...,(A_n,\omega_n))$
    \end{center}
    
    Wykonawca $\omega$ jest zawsze/kiedykolwiek zaangażowany w realizację programu działań $((A_1, \omega_1),...,(A_n,\omega_n))$.    
    
    \subsection{Semantyka}
    
    \begin{definition}[Konsekwencja]
    	Niech $D$ będzie dziedziną akcji języka $\mathcal{ADA}$. $Q$ jest konsekwencją $D$ (ozn. $D | \approx Q$), wtedy i tylko wtedy kiedy:
    
    \begin{itemize}
    	\item jeżeli $Q$ jest postaci $\mathbf{always}/\mathbf{ever}~\mathbf{executable}~((A_1, \omega_1),...,(A_n,\omega_n))$: \\ dla każdej struktury $S=(\Sigma, \sigma_0, ResAb, ResN)$, będącej modelem $D$, program działań $((A_1, \omega_1),...,(A_n,\omega_n))$ jest wykonywalny.
    	
    	\item jeżeli $Q$ jest postaci $\mathbf{always}/\mathbf{ever}~\gamma~\mathbf{if}~\pi~\mathbf{when}~((A_1, \omega_1),...,(A_n,\omega_n))$: \\
    	...
    	\item jeżeli $Q$ jest postaci $\mathbf{typically}~\gamma~\mathbf{if}~\pi~\mathbf{when}~((A_1, \omega_1),...,(A_n,\omega_n))$: \\
    	...
    	\item jeżeli $Q$ jest postaci $\mathbf{always}/\mathbf{ever}~\gamma~\mathbf{if}~\pi$: \\
    	...
    	\item jeżeli $Q$ jest postaci $\mathbf{typically}~\gamma~\mathbf{if}~\pi$: \\
    	...
    	\item jeżeli $Q$ jest postaci $\mathbf{always}/\mathbf{ever}~\omega~\mathbf{in}~((A_1, \omega_1),...,(A_n,\omega_n))$: \\
    	 dla każdej struktury $S=(\Sigma, \sigma_0, ResAb, ResN)$, będącej modelem $D$, $\omega$ jest wykonawcą w jednym z elementów ciągu $((A_1, \omega_1),...,(A_n,\omega_n))$ lub istnieje element tego ciągu $(A_i, \epsilon),~1 \leq i \leq n$, taki że akcja $A_i$ jest wykonywalna przez wykonawcę $\omega$ w stanie otrzymanym po wykonaniu planu działania $((A_1, \omega_1),...,(A_{i-1},\omega_{i-1}))$ lub w stanie $\sigma_0$ jeżeli $i=1$.
    \end{itemize}
    \end{definition}
