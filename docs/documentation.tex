...
\begin{document}
    ...
    \begin{itemize}
    \item zbiór agentów: $\Omega$ = \{FA, BA\} - Frodo, Bilbo
    \item zbiór akcji: $A_c$ = \{PM, ZF, ZB\} - ''Podnieś miecz'', ''Zaatakuj Froda'', ''Zaatakuj Bilba''
    \item zbiór fluentów: $\mathcal{F}$ = \{F, B, FM, BM\} - Frodo żyje, Bilbo żyje, Frodo trzyma miecz, Bilbo trzyma miecz
    ...
    \}.
    \end{itemize}
    Ponadto w języku znajdują się następujące zdania:
    
    \begin{center}
    $\mathbf{initially}$ F $\wedge$ B $\wedge$ $\neg$FM $\wedge$ $\neg$BM\\
    $\mathbf{when}$ FA PM $\mathbf{causes}$ FM $\mathbf{if}$ $\neg$BM\\
    $\mathbf{impossible}$ FA ZF\\
    $\mathbf{impossible}$ BA ZB\\
    ZF $\mathbf{typically} ~ \mathbf{causes}~\neg$F $\mathbf{if}$ BM\\
    ZB $\mathbf{causes}~\neg$B $\mathbf{if}$ FM\\
    $\mathbf{observable}~\neg$B $\mathbf{after}$ ZB\\
